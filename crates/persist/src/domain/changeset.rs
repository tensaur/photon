...
impl ChangeSet {
    ...
    /// Mark a run as finalised. Queues a `Finalised` event.
    pub fn mark_finalised(&mut self, run_id: RunId) {
        self.finalised_runs.push(run_id);
        self.events.push(PhotonEvent::Finalised { run_id });
    }
