...
impl Aggregator for NoOpAggregator {
    ...
    fn new_bucket(&self, _step: Step, value: f64) -> NoOpBucket {
        NoOpBucket { value }
    }

    fn push(&self, bucket: &mut NoOpBucket, _step: Step, value: f64) {
        bucket.value = value;
    }
    ...
    fn close(&self, bucket: &NoOpBucket, step_start: Step, step_end: Step) -> Bucket {
        Bucket {
            step_start,
            step_end,
            value: bucket.value,
